...
var PRIORITY_FIELD_REGEX = /\[\s*priority\s*::\s*([^\]]+?)\s*\]/gi;
var PRIORITY_FIELD_STRIP_REGEX = /\s*\[\s*priority\s*::[^\]]*?\]/gi;
function parseDataviewPriority(line) {
...
function stripDataviewPriorityFields(line) {
  return line.replace(PRIORITY_FIELD_STRIP_REGEX, "").replace(/[ \t]{2,}/g, " ").trim();
}
function sortTasksByPriority(tasks) {
  return tasks.map((task, index) => ({ task, index })).sort((a, b) => {
    var _a, _b;
    const pa = (_a = a.task.priority) != null ? _a : 99;
    const pb = (_b = b.task.priority) != null ? _b : 99;
    if (pa !== pb)
      return pa - pb;
