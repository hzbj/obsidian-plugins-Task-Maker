...
import { stripDataviewPriorityFields } from './TaskPriorityService';
...
export class TagManagerService {
	...
	/** Build a regex that strips legacy priority tags with the current namespace */
	private buildLegacyPriorityRegex(): RegExp {
		const prefix = this.escapeRegex(this.getPrefix());
	...
	cleanDisplayText(rawLine: string, triggerTags: string[]): string {
		...
		// Remove legacy priority tags and current Dataview priority fields.
		const priorityRegex = this.buildLegacyPriorityRegex();
		text = text.replace(priorityRegex, '');
		text = stripDataviewPriorityFields(text);
